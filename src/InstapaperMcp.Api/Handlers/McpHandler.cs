...
public sealed class McpHandler
{
    ...
    public McpHandler(IInstapaperService instapaperService, ILogger<McpHandler> logger)
    {
    ...
    private object ListTools()
    {
        return new
        {
            tools = new[]
            {
                ...
                new { name = "manage_bookmarks", description = "Archive, unarchive, delete, star, or unstar bookmarks. Action is applied to all provided IDs." },
    ...
    {
        long? folderId = args.TryGetProperty("folder_id", out var f) ? f.GetInt64() : null;
        string? query = args.TryGetProperty("query", out var q) ? q.GetString() : null;
        int limit = args.TryGetProperty("limit", out var l) ? l.GetInt32() : 10;

        var result = await _instapaperService.SearchBookmarksAsync(folderId, query, limit, ct);
        return FormatResult(result);
    }

    private async Task<object> AddBookmarkAsync(JsonElement args, CancellationToken ct)
    {
        string? url = args.TryGetProperty("url", out var u) ? u.GetString() : null;
        string? content = args.TryGetProperty("content", out var c) ? c.GetString() : null;
        string? title = args.TryGetProperty("title", out var t) ? t.GetString() : null;
        string? description = args.TryGetProperty("description", out var d) ? d.GetString() : null;
        long? folderId = args.TryGetProperty("folder_id", out var f) ? f.GetInt64() : null;

        var result = await _instapaperService.AddBookmarkAsync(url, content, title, description, folderId, ct);
        return FormatResult(result);
    }
    ...
    private async Task<object> ManageBookmarksAsync(JsonElement args, CancellationToken ct)
    {
        ...
        var result = await _instapaperService.ManageBookmarksAsync(ids, action, ct);
        return FormatResult(result);
    }
    ...
    private async Task<object> MoveBookmarksAsync(JsonElement args, CancellationToken ct)
    {
        ...
        var result = await _instapaperService.MoveBookmarksAsync(ids, folderId, ct);
        return FormatResult(result);
    }
    ...
    private async Task<object> ManageFoldersAsync(JsonElement args, CancellationToken ct)
    {
        ...
        {
            "list" => FormatResult(await _instapaperService.ListFoldersAsync(ct)),
            "create" => FormatResult(await _instapaperService.CreateFolderAsync(args.GetProperty("title").GetString()!, ct)),
            "delete" => FormatResult(await _instapaperService.DeleteFolderAsync(args.GetProperty("folder_id").GetInt64(), ct)),
    ...
    private async Task<object> ManageHighlightsAsync(JsonElement args, CancellationToken ct)
    {
        ...
        return action switch
        {
            "list" => FormatResult(await _instapaperService.ListHighlightsAsync(args.GetProperty("bookmark_id").GetInt64(), ct)),
            "add" => FormatResult(await _instapaperService.AddHighlightAsync(args.GetProperty("bookmark_id").GetInt64(), args.GetProperty("text").GetString()!, args.TryGetProperty("note", out var n) ? n.GetString() : null, ct)),
            "delete" => FormatResult(await _instapaperService.DeleteHighlightAsync(args.GetProperty("highlight_id").GetInt64(), ct)),
    ...
    private object ListResources()
    {
        return new
        {
            resources = new[]
            {
                new { uri = "instapaper://folders", name = "Application folders", description = "List of all user folders" },
                new { uri = "instapaper://bookmarks/unread", name = "Unread bookmarks", description = "Default bucket for new items" }
            }
        };
    }
    ...
    private async Task<object> ReadResourceAsync(JsonElement? parameters, CancellationToken ct)
    {
        ...
        {
            var result = await _instapaperService.ListFoldersAsync(ct);
    ...
    private object FormatResult(Result result) => result.IsSuccess
    ...
    private object FormatResult<T>(Result<T> result) => result.IsSuccess
        ? new { content = new[] { new { type = "text", text = JsonSerializer.Serialize(result.Value, new JsonSerializerOptions { WriteIndented = true }) } } }
        : new { isError = true, content = new[] { new { type = "text", text = result.Error } } };
}
